...
CSocket::CSocket()
{
	...
	if (err != 0 || LOBYTE(wsaData.wVersion) != 2 || HIBYTE(wsaData.wVersion) != 2) {
		printf("Winsock failed to initialize: %d\n", WSAGetLastError());
		this->~CSocket();
	}
}
...
int CSocket::set_max_connections(int socketid, int maxcon)
{
	if(m_pSocket[socketid] == -1 || !m_pSocketInfo[socketid].success) {
		//printf("set_max_connections(): Socket ID %d doesn't exist or hasn't been created yet", socketid);
		return 0;
	}
...
{
	if(m_pSocket[socketid] == -1 || !m_pSocketInfo[socketid].success) {
		//printf("socket_connect(): Socket ID %d doesn't exist or hasn't been created yet", socketid);
		return 0;
	}
	...
	/*if(strlen(m_pSocketInfo[socketid].bind_ip) > 0) {
		...
		if(bind(m_pSocket[socketid], (struct sockaddr *)&bind_addr, sizeof(bind_addr)) == -1) {
			printf("socket_connect(): Socket ID %d has failed to bind IP %s.", socketid, m_pSocketInfo[socketid].bind_ip);
			return 0;
		}
	} */
	if(connect(m_pSocket[socketid], (struct sockaddr *)&addr, sizeof(sockaddr)) == SOCKET_ERROR) {
		//printf("socket_connect(): Socket ID %d has failed to connect.", socketid);
		return 0;
	}
...
{
	if(m_pSocket[socketid] == -1 || !m_pSocketInfo[socketid].success) {
		//printf("socket_listen(): Socket ID %d doesn't exist or hasn't been created yet", socketid);
		return 0;
	}
	struct sockaddr_in addr;
	addr.sin_family = AF_INET;
	...
	if(bind(m_pSocket[socketid], (struct sockaddr *)&addr, sizeof(addr)) == -1) {
		//printf("socket_listen(): Socket has failed to bind on port %d.", port);
		return 0;
	}
	if(m_pSocketInfo[socketid].tcp) { 
		if(listen(m_pSocket[socketid], 10) == -1) {
			//printf("socket_listen(): Socket has failed to listen on port %d.", port);
			return 0;
		}
	}
...
{
	if(m_pSocket[socketid] == -1 || !m_pSocketInfo[socketid].success) {
		//printf("stop_listen_socket(): Socket ID %d doesn't exist or hasn't been created yet", socketid);
		return 0;
	}
...
{
	if(m_pSocket[socketid] == -1 || !m_pSocketInfo[socketid].success) {
		//printf("socket_bind(): Socket ID %d doesn't exist or hasn't been created yet", socketid);
		return 0;
	}
...
{
	if(m_pSocket[socketid] == -1 || !m_pSocketInfo[socketid].success) {
		//printf("socket_destroy(): Socket ID %d doesn't exist or hasn't been created yet", socketid);
		return 0;
	}
...
{
	if(m_pSocket[socketid] == -1 || !m_pSocketInfo[socketid].success) {
		//printf("close_remote_connection(): Socket ID %d doesn't exist or hasn't been created yet", socketid);
		return 0;
	}
...
{
	if(m_pSocket[socketid] == -1 || !m_pSocketInfo[socketid].success) {
		//printf("socket_send(): Socket ID %d doesn't exist or hasn't been created yet", socketid);
		return 0;
	}
	//strcat(data, "\r\n");
...
{
	if(m_pSocket[socketid] == -1 || !m_pSocketInfo[socketid].success) {
		//printf("sendto_remote_client(): Socket ID %d doesn't exist or hasn't been created yet", socketid);
		return 0;
	}
...
{
	if(m_pSocket[socketid] == -1 || !m_pSocketInfo[socketid].success) {
		//printf("is_remote_client_connected(): Socket ID %d doesn't exist or hasn't been created yet", socketid);
		return 0;
	}
	char tempbuf[1024];
...
{
	if(m_pSocket[socketid] == -1 || !m_pSocketInfo[socketid].success) {
		//printf("get_remote_client_ip(): Socket ID %d doesn't exist or hasn't been created yet", socketid);
		return 0;
	}
...
#endif
{
	...
	// printf("Thread started with socket id %d", sockID);
